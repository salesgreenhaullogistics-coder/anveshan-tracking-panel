...
export default function GlobalSearch() {
  ...
  return (
    <div ref={wrapperRef} className="relative w-full">
      <div className="flex items-stretch rounded-lg border-2 border-slate-200 bg-white shadow-sm overflow-hidden hover:border-slate-300 transition-colors">
        ...
        <input
          ...
          placeholder="Search AWB, Invoice, Ref. No..."
        ...
        />
          {query && <button onClick={() => { setQuery(''); setResult(null); setOpen(false); }} className="text-slate-400 hover:text-slate-600 flex-shrink-0"><X className="w-4 h-4" /></button>}
        </div>
        <button
          onClick={() => executeSearch()}
          title="Execute search (Enter)"
          className="px-3.5 border-l border-slate-200 bg-slate-50 hover:bg-slate-100 transition-colors flex items-center gap-2 flex-shrink-0"
        >
